...
class ReadingProgressController extends GetxController {
  ...
  final RxMap<String, ReadingProgress> bookProgress =
  ...
  Future<void> endReadingSession({
    ...
    try {
      ...
      print(
          'DEBUG: Saving reading session - Duration: ${duration.inMinutes} minutes');
  ...
  Future<void> updateBookProgress(
    ...
    try {
      ...
      print(
          'DEBUG: Progress calculation - Current: $currentPage, Total: $totalPages, Percent: $progressPercent');
  ...
  Future<void> _calculateStatistics() async {
    try {
      ...
      totalBooksRead.value =
          bookProgress.values.where((progress) => progress.isCompleted).length;
      ...
      for (var session in readingSessions) {
        if (session.startTime.isAfter(thirtyDaysAgo)) {
          ...
          dailyReadingMinutes[dateKey] =
              (dailyReadingMinutes[dateKey] ?? 0) + minutes;
        }
      }
      ...
      averageReadingSpeed.value =
          totalMinutes > 0 ? totalWords / totalMinutes : 0;
    } catch (e) {
